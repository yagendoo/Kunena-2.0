...
/*			if ($this->me->isAdmin($this->category)) {
	...
	function getCategoryActions() {
		...
		if ($this->category->getNewTopicCategory()->exists()) {
			...
			$this->categoryButtons['create']	=	'<a class="btn btn-primary" href="'.$url.'" rel="nofollow">'.JText::_('COM_KUNENA_CREATE_TOPIC').'</a>';
		}
		...
		if ($this->me->exists() && $this->total) {
			...
			$this->categoryButtons['markread']	=	'<a class="btn" href="'.$url.'" rel="nofollow">'.JText::_('COM_KUNENA_MARKASREAD').'</a>';
		}
		...
		if ($this->category->authorise ( 'subscribe', null, true )) {
			...
			if (!$subscribed) {
				...
				$this->categoryButtons['subscribe']	=	'<a class="btn" href="'.$url.'" rel="nofollow">'.JText::_('COM_KUNENA_SUBSCRIBE').'</a>';
			} else {
				...
				$this->categoryButtons['unsubscribe']	=	'<a class="btn" href="'.$url.'" rel="nofollow">'.JText::_('COM_KUNENA_UNSUBSCRIBE').'</a>';
			}
		}
